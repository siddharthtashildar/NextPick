...
const Navbar = () => {
  ...
  useEffect(() => {
    if (searchQuery.trim() !== "") {
      Searchnavigate(`/search?q=${encodeURIComponent(searchQuery)}`);
    }
  }, [searchQuery]);

  useEffect(() => {
    // Only focus when on /search route
  ...
  return (
    <header className="fixed top-0 left-0 w-full h-5 flex justify-between items-center mb-12 py-1 pl-10 pr-10 mt-10 z-1000 ">

      <Link to="/"><h2 className="text-3xl font-extrabold">NextPick</h2></Link>
      ...
      <div className="relative text-gray-600 focus-within:text-gray-400 ">
        ...
        <input
          ...
          className="py-2 w-130 h-9 text-sm text-white bg-gray-900 opacity-75 rounded-md pl-10 focus:outline-none focus:bg-black focus:border-1 focus:text-gray-200"
